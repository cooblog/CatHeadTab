...
export const TrendingModal: React.FC<TrendingModalProps> = ({ type, onClose }) => {
  ...
  const title = isGithub ? 'GitHub Trending' : (isZh ? '哔哩哔哩热门' : 'Bilibili Hot');
  const accentColor = isGithub ? '#72d565' : '#00A1D6';

  const load = useCallback(async () => {
    if (!serverUrl) return;
    setLoading(true);
    try {
      const endpoint = isGithub ? 'github' : 'bilibili';
  ...
  return (
    <div className={`fixed inset-0 z-[100] flex items-center justify-center pointer-events-none ${isFullscreen ? 'p-0' : 'p-0 sm:p-6 md:p-12'}`}>
      ...
      >
        ...
        {/* Body */}
        <div className="flex-1 overflow-y-auto no-scrollbar px-2 sm:px-3 py-3 select-text">
          ...
          {/* GitHub list */}
          {isGithub && (data as TrendingRepo[]).map((repo, i) => (
            <a
              key={repo.fullName}
              href={repo.url}
              target="_blank"
          ...
          {/* Bilibili list */}
          {!isGithub && (data as HotVideo[]).map((video, i) => (
            <a
              key={video.bvid}
              href={video.url}
              target="_blank"
